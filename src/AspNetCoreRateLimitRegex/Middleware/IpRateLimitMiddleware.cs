...
namespace AspNetCoreRateLimitRegex
{
    public class IpRateLimitMiddleware
    {
        ...
        {
            _logger.LogInformation($"Request {identity.HttpVerb}:{identity.Path} from IP {identity.ClientIp} has been blocked, quota {rule.Limit}/{rule.Period} exceeded by {counter.TotalRequests}. Blocked by rule {rule.UrlRegex}, TraceIdentifier {httpContext.TraceIdentifier}.");
        }
